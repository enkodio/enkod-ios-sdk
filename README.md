...
### Метод sendCustomEvent

Данный метод позволяет отправлять пользовательские события в систему Enkod. Используйте его, когда вам необходимо отслеживать специфические действия пользователей, которые не покрываются стандартными событиями трекинга.

#### Параметры:

- `name: String` - Название события. Обязательный параметр, не может быть пустой строкой.
  
- `parameters: [String: Any]` - Дополнительные параметры события в виде словаря. Используйте для передачи любых дополнительных данных, связанных с событием.
  
- `email: String? = nil` - Email пользователя. Опциональный параметр, но должен быть указан либо email, либо phone.
  
- `phone: String? = nil` - Телефон пользователя. Опциональный параметр, но должен быть указан либо email, либо phone.
  
- `completion: @escaping (Result<Void, EnkodCustomEventError>) -> Void` - Обработчик завершения запроса, который возвращает результат операции:
  - `.success(())` - Если событие успешно отправлено
  - `.failure(EnkodCustomEventError)` - В случае ошибки, с типом ошибки

#### Возможные ошибки (EnkodCustomEventError):

- `.invalidParameters` - Параметры события недействительны (пустое имя события или не указан ни email, ни телефон)
- `.networkError(Error)` - Ошибка сети при отправке события
- `.invalidResponse` - Некорректный ответ от сервера
- `.serverError(String)` - Ошибка сервера с описанием
- `.unknown` - Неизвестная ошибка

#### Пример использования:

```swift
// Отправка пользовательского события "registration_completed"
EnkodSdk.shared.sendCustomEvent(
    ...
    completion: { result in
        ...
        case .failure(let error):
            print("Ошибка при отправке события: \(error)")
        }
    }
)
```

> **Важно:** Для успешной отправки события необходимо указать либо email, либо телефон пользователя, а также непустое имя события.
